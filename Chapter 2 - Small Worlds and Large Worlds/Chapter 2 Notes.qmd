...
posterior <- map2(
    ...
    .y = data$n_successes,
    .f = \(trial, n) {
        likelihood <- dbinom(n, trial, p_water)
        tibble(p_water = p_water, 
...
prior <- posterior |>
    ...
    mutate(
        trial = lag(trial, n = 1),
        likelihood = lag(likelihood, n = 1),
        trial = trial + 1
    ) |>
    ungroup() |>
    mutate(
        trial = if_else(is.na(trial), 1, trial),
        likelihood = ifelse(trial == 1, 0.5, likelihood),
        likelihood = ifelse(trial == 1, likelihood / sum(likelihood[trial == 1]), likelihood)
    ) |>
    rename(prior = likelihood)

df <- posterior |>
    bind_cols(prior |> select(prior))
```
...
posterior |>
    ggplot(aes(x = p_water, y = likelihood)) +
    geom_line() +
    facet_wrap(~trial, scales = "free_y")
